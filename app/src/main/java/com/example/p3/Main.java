...
public class Main extends AppCompatActivity {
    ...
    public static String NICKNAME = Registration.NICKNAME;
    ...
    static String message2Send = "";
    static boolean isMessage = false;
    File log = new File("/data/user/0/com.example.p3/files/log.txt");
                    ...
                    public boolean onItemClick(@Nullable View view, int i, @NotNull IDrawerItem<?> iDrawerItem) {
                        switch (i) {
                            ...
                            case 3:
                                ...
                                toolbar.setTitle("Chat Roulette");
                                changeView(3);
                                break;
                            case 5:
    ...
    public void updateList(String signal) {
        ...
        String peerName = signal.substring(i1, i - 11);
        users.add(peerName);

        if (se.equals(location)) {
            if (location.equals("3")) {
                ...
                if (peerName.trim().compareToIgnoreCase(NICKNAME) < 0) {
                    location = NICKNAME;
                    toolbar.setTitle(peerName.trim());
                } else {
                    location = peerName.trim();
            ...
            } else if (location.length() > 2 && location.substring(0, 3).equals("2||")) {
                if (mViewPager.getCurrentItem() != 1) {
                    GlobalChatFragment.cleanLog();
                }
                mViewPager.setCurrentItem(1);
            ...
            try {
                if (signal.substring(iii + 1, iii + 2).equals("M")) {
                    Log.e(TAG, signal.substring(iii + 1, iii + 2));
                    FileWriter fileWriter = new FileWriter(log, true);

                    BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
                    String writeLog = signal.substring(iii + 2);
            ...
            try {
                Log.e(TAG, se + " " + se.substring(0, 3));
                if (se.substring(0, 3).equals("2||")) {
                    ChatRoomsFragment.items.add(se.substring(3));
                }
            } catch (Exception e) {
    ...
    public void distribute2All(final String messageFromGlobalChat) {
        for (int i = 0; i < clients.size(); i++) {
            try {
                final int index = i;
                new Thread("distributionMessage") {
                    @Override
                    public void run() {
                        ...
                        String M = "I'm: " + NICKNAME + ",my Mac ad:" + macAddress + " ,at: " + location + "#";
                        M += messageFromGlobalChat;
                ...
                Log.e(TAG, "distribution Exception " + e.getMessage());
            }
        }
        ...
        for (int i = 0; i < clients.size(); i++) {
            try {
                ...
                new Thread("aliveMessage") {
                    ...
                    public void run() {
                        sendReceive2 = clients.get(index);
                        String M = "I'm: " + NICKNAME + ",my Mac ad:" + macAddress + " ,at: "
                                + location + "#";
                        boolean intactPipe = sendReceive2.write(M.getBytes());
                        if (!intactPipe) {
                            clients.remove(index);
